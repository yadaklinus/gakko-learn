            ...
            {dashboardData?.bookingsAsStudent[0] ? (
              ...
              <>
                ...
                <button
                  ...
                  className="mt-auto w-full bg-indigo-50 text-indigo-600 py-3 rounded-2xl font-bold text-sm hover:bg-indigo-100 transition-all"
                >
                  Find a Tutor
                </button>
              </>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <section>
          ...
          <div className="space-y-4">
            ...
            {!isLoading && (!dashboardData?.topRatedTutors || dashboardData.topRatedTutors.length === 0) && (
              <p className="text-slate-400 text-sm italic">No ratings available yet.</p>
            )}
            {dashboardData?.topRatedTutors?.map((tutor) => (
              <div key={tutor.id} className="bg-slate-50 border border-slate-100 p-4 rounded-3xl flex items-center space-x-4 hover:bg-white hover:shadow-md transition-all cursor-pointer">
                <div className="w-12 h-12 rounded-xl overflow-hidden shadow-sm">
                  <img src={tutor.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(tutor.name)}&background=random`} alt={tutor.name} className="object-cover w-full h-full" />
                </div>
                <div className="flex-1">
                  <h4 className="font-bold text-sm text-slate-900 truncate">{tutor.name}</h4>
                  <p className="text-[10px] text-slate-500 font-medium truncate">
                    {tutor.major || 'Peer Tutor'}
                  </p>
                </div>
                <div className="flex flex-col items-end">
                  ...
                  <span className="text-[8px] text-slate-400 font-bold">{tutor.totalReviews} reviews</span>
                </div>
              </div>
            ))}
          </div>
